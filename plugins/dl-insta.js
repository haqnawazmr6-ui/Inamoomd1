...
cmd({
  ...
  try {
    ...
    const apiUrl = `https://NAWAZ-MD.vercel.app/downloader?url=${encodeURIComponent(igUrl)}`;
    const response = await axios.get(apiUrl);
...
cmd({
    ...
    try {
        ...
        // Fetch from your API  
        const apiUrl = `https://NAWAZ-MD.vercel.app/igdl?url=${encodeURIComponent(url)}`;
        const response = await axios.get(apiUrl);
